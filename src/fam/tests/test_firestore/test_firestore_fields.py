...
class TestDB(unittest.TestCase):
    ...
    def test_geopoint(self):
        ...
        loc = GeoPoint(latitude=51.2345, longitude=-1.4533)
        house = House.create(self.db, name="my house", location=loc)
        house2 = House.get(self.db, house.key)
        self.assertTrue(type(house2.location) == GeoPoint)
        self.assertEqual(house2.location.latitude, 51.2345)


    def test_polyline(self):

        loc1 = GeoPoint(latitude=51.2345, longitude=-1.4533)
        loc2 = GeoPoint(latitude=51.3345, longitude=-1.4533)
        loc3 = GeoPoint(latitude=51.3345, longitude=-1.3533)
        loc4 = GeoPoint(latitude=51.2345, longitude=-1.3533)

        fence = Fence.create(self.db, name="my house", boundary=[loc1, loc2, loc3, loc4])

        self.assertTrue(type(fence.boundary[2]) == GeoPoint)
